...
def test_controller_only_preempts_speculation_for_trusted_broad_portfolio_pressure(
    ...
    controller.put_profile(old)
    controller.put_profile(coder)
    ...
    assert controller.last_plan is not None
    assert controller.last_plan.urgency_for("coder") == (2 if attested else 0)
    assert bool(controller.last_plan.preemptions) is expect_preemption
    if expect_preemption:
        assert [
            (item.model_id, item.for_model_id)
            for item in controller.last_plan.preemptions
        ] == [("old-speculation", "coder")]
