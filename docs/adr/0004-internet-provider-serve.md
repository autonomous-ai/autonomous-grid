# ADR 0004 — Unified `grid join`: internet provider serve (single engine)
...
— the one verb that makes a machine serve models — work in **internet mode**, and lands the **unified
`grid join` flag set**.

In LAN, `grid join` writes an engine record and spawns a detached `__engine` child that push-registers
the in-memory grid proxy and heartbeats it. Internet is the same verb with a different loop: bring the
...
1. **New internal `__internet-engine <grid_id> <engine_id>` seam.** Dispatched in `cli/_main.py:_maybe_internal`
   into a new `internet/serve.py`; LAN's `__engine` → `cli/provider._run_engine` is untouched. The mode
...
2. **The engine record + `grid leave` are shared.** Record I/O and the SIGTERM→SIGKILL teardown moved
   ...
   existing test surface are byte-identical. Internet (`internet/serve.py`, `cli/internet_provider.py`) reuses the
   same module — no `internet → cli` back-dependency.
...
5. **Full live capability probe.** `internet/probe.py` (ported from the reference `engine/probe.py` +
...
6. **Relay contract = the reference paths**, base = the grid's `signaling_url`, `Authorization: Bearer
   ...
   `internet/relay.py` uses a **status-aware** policy (204/401/404 are outcomes, not fatal) — distinct from
...
7. **One engine, gated flags.** Internet `--all` and multi-engine routing (US 27) are rejected with
   guidance; auto-detect finding >1 engine errors. `--media` serving in internet mode is rejected for now (a later
   slice). The unified parser is one union; each handler validates per mode using the stamped `args.mode`
   seam: LAN `cmd_join` rejects the internet-only flags, internet `cmd_internet_join` rejects `--advertise-host` /
   `--media`. Internet-only flags default `None` so a wrong-mode use is detectable. **`--node-name` is
...
8. **`grid join` runs detached; the grid must be up.** `cmd_internet_join` resolves the active grid (reusing
   `cli/internet_grid`'s selection), reads the live `signaling_url` from `…/status`, and refuses to spawn if
   the grid isn't running or has no relay address (a clear "run `grid up` first" instead of a background
   loop that can never register). It writes the shared record, spawns `__internet-engine`, and confirms only
...
- LAN is untouched: the extraction is a pure refactor, the internet-only flags are rejected in LAN, and no
...
- The detached seam is the single plug-in point a future multi-engine / internet-media slice extends; the
