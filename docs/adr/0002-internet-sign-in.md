# ADR 0002 — Internet sign-in (`grid login` / `grid logout`)

Status: accepted (2026-06-27); amended 2026-06-29 — added Decision 11 (`grid sync`, issue 10)

## Context

ADR 0001 set up modes, `state.json`, and mode-aware dispatch with internet as a stub. This slice
fills the first real internet surface: **sign-in**. `grid login` runs the Google device-code flow
...
gate that every later internet command (request/info/members) will reuse.
...
Hard invariant: LAN mode stays LAN-only, unauthenticated, stateless — unchanged. Internet mode is
*allowed* to reach the internet to the control plane; that is the feature.

## Decisions

1. **Internet-only commands gated symmetrically (`cli/dispatch.py`).** A new `INTERNET_ONLY =
   {"login", "logout"}` set joins `AGNOSTIC` and `INTERNET_HANDLERS`; the three now partition every
   registered command (the classification test asserts coverage *and* disjointness). In internet mode
   `INTERNET_ONLY` falls through to its real handler; in LAN mode it hits `lan_stub` — the mirror of
   the existing `internet_stub`. The LAN gate is an **`elif`** after the `if mode == "internet"` block:
   `dispatch` has no `else`, so a bare `if` would fire in internet mode too and break login/logout there.

2. **Device-code flow, ported and trimmed (`internet/control_plane.py`).** Three functions only —
...
6. **Login never auto-selects an active grid (supersedes D10).** D10 had `grid login` write the
   active internet grid; we deliberately don't. Login lists the available grids and instructs
...
7. **Re-login refreshes by re-auth; no silent refresh.** Re-running `grid login` performs a fresh
   ...
   (YAGNI). `grid logout` deletes `credentials.toml` and clears `active.internet`; it keeps
...
10. **The auth gate: `credentials.require_session()`.** Returns the stored session token or raises
    `SystemExit("You're not signed in. Run `grid login` to sign in.")`. No auth-requiring internet
...
11. **`grid sync` — refresh the grid list without re-login (amendment 2026-06-29, issue 10).**
    ...
    **session** rejected by the control plane (401/403) is rewritten to an actionable "run `grid
    ...
    control-plane error propagates unchanged. Surface: `sync` joins `INTERNET_ONLY` (now `{"login",
    ...
    unrelated to `DECISIONS.md`'s D11, the managed-networks repoint cited in `internet/control_plane.py`.)
...
- A future internet-only command added without placing it in `INTERNET_ONLY` fails the classification
  test rather than silently running LAN code (or erroring opaquely) in the wrong mode.
- The control plane is reachable only in internet mode and only via `internet/`; `shared/`/`lan/` gain no
  off-LAN calls.
- `grid sync` (amendment) makes `INTERNET_ONLY` `{"login", "logout", "sync"}`; the classification test
