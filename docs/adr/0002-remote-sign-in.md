...
ADR 0001 set up modes, `state.json`, and mode-aware dispatch with remote as a stub. This slice
fills the first real remote surface: **sign-in**. `grid login` runs the Google device-code flow
...
gate that every later remote command (request/info/members) will reuse.
...
Hard invariant: local mode stays local-only, unauthenticated, stateless — unchanged. Remote mode is
*allowed* to reach the remote to the control plane; that is the feature.

## Decisions

1. **Remote-only commands gated symmetrically (`cli/dispatch.py`).** A new `REMOTE_ONLY =
   {"login", "logout"}` set joins `AGNOSTIC` and `REMOTE_HANDLERS`; the three now partition every
   registered command (the classification test asserts coverage *and* disjointness). In remote mode
   `REMOTE_ONLY` falls through to its real handler; in local mode it hits `local_stub` — the mirror of
   the existing `remote_stub`. The local gate is an **`elif`** after the `if mode == "remote"` block:
   `dispatch` has no `else`, so a bare `if` would fire in remote mode too and break login/logout there.

2. **Device-code flow, ported and trimmed (`remote/control_plane.py`).** Three functions only —
...
6. **Login never auto-selects an active grid (supersedes D10).** D10 had `grid login` write the
   active remote grid; we deliberately don't. Login lists the available grids and instructs
...
9. **Dependencies: `tomli-w` only, in base.** Writing TOML needs `tomli-w` (added to base
   `dependencies`, not an extra, so local-only use is unaffected); reads use stdlib `tomllib`.
...
10. **The auth gate: `credentials.require_session()`.** Returns the stored session token or raises
    `SystemExit("You're not signed in. Run `grid login` to sign in.")`. No auth-requiring remote
...
11. **`grid sync` — refresh the grid list without re-login (amendment 2026-06-29, issue 10).**
    ...
    **session** rejected by the control plane (401/403) is rewritten to an actionable "run `grid
    ...
    unrelated to `DECISIONS.md`'s D11, the managed-networks repoint cited in `remote/control_plane.py`.)

## Consequences

- local mode is untouched: login/logout are gated with guidance, and no new code runs for local users.
...
- A future remote-only command added without placing it in `REMOTE_ONLY` fails the classification
  test rather than silently running local code (or erroring opaquely) in the wrong mode.
- The control plane is reachable only in remote mode and only via `remote/`; `shared/`/`local/` gain no
  off-local calls.
