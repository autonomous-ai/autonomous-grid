# ADR 0007 — Remote multi-engine routing (`grid join --all`, one identity, model→engine)

Status: accepted (2026-06-28)

## Context

ADR 0004 stood up the remote provider serve loop for **one** engine and §7 deliberately deferred
multi-engine: `--all` was rejected and auto-detecting >1 engine errored. This slice flips that guard
so one remote identity can serve several local engines at once (DECISIONS **D9**; PRD User Story 27).
...
model. Builds directly on ADR 0004 (the `__remote-engine` loop, relay contract, refresh-on-401) and
...
Hard invariant: local stays local-only / unauthenticated / stateless; single-engine remote serve and the
whole existing suite stay green; remote reaches the relay only through `remote/`; tokens are never
...
1. **One `grid join` = one identity; `--all` is the union mechanism; repeated joins stay separate.**
   ...
   detached `__remote-engine` process. `--all` (or an interactive "join all" confirm) gathers every
...
2. **Routing is client-side on `body["model"]`; first-detected wins.** `remote/serve._build_routing`
...
4. **Multi-engine is external-only; the built-in launch stays single-engine.** `--all` only gathers
   ...
   **advertised** name and the body is forwarded unchanged, so advertised must equal the engine's real
   ...
   launch, which detected external engines have no hook for. Remote media engines are still rejected (a
   lone one) or skipped with a note (under `--all`); remote media serving remains a later slice.
...
- Single-engine remote serve is unchanged: `_ServeState` still constructs from `llm_url` + `models`
...
- local is untouched. The routing + bring-up live entirely in `remote/serve.py` + `cli/remote_provider.py`;
