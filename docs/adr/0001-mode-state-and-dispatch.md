...
proxy) and `internet` (a future thin client to autonomous's hosted relay).
...
grid is selected per mode, and how command handlers become mode-aware while internet is still a
stub.
...
1. **Mode-aware dispatch via a central table (`cli/dispatch.py`).** LAN handlers stay wired in the
   ...
   `INTERNET_HANDLERS` — classify every command; a test asserts the union covers the parser's commands
   (fail-loud, never fail-open). Internet entries are stubs now and become real handlers in later slices.
...
4. **Per-invocation override `--lan` / `--internet` > persisted mode > default `lan`.** The flags are
   stripped from `argv` before parsing so they work in any position; specifying both is an error.

5. **Internet is a clear stub this slice.** `grid mode internet` switches and persists (per the issue's
   ...
   crashing. Bare `grid` in internet mode shows the mode + active + how to switch, with no network calls.
...
6. **Command classification.** Mode-agnostic (run unchanged in both modes): `version`, `catalog`,
   ...
   display, but never gated). Mode-gated (internet → stub now): `up`, `down`, `ls`/`list`, `info`,
...
- The dispatch table is the single seam later internet slices plug real handlers into.
...
- A future LAN-only command added without classifying it in `AGNOSTIC`/`INTERNET_HANDLERS` fails the
  coverage test rather than silently running LAN code in internet mode.
