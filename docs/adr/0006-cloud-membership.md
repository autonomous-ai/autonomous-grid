# ADR 0006 — Cloud membership (`grid members add` / `remove` / `list`)

Status: accepted (2026-06-28)

## Context

ADR 0001 set up modes + dispatch; 0002 sign-in + the account `session_token`; 0003 the cloud grid
lifecycle (`grid up/down/ls/info`) authenticated with that session token; 0004 the provider serve
loop; 0005 the consume path. This slice fills the last cloud surface in scope: **membership
administration** for a cloud grid you own. `grid members add [grid] <email> [--role …]`,
...
suite stays green; cloud reaches the control plane only through `cloud/`; tokens are never printed.
...
1. **`members` is a `CLOUD_ONLY` command with nested subcommands.** `"members"` joins
   `dispatch.CLOUD_ONLY` (with `login`/`logout`). In cloud mode dispatch falls through to
   `args.handler`; in LAN mode the `elif command in CLOUD_ONLY → lan_stub` gate fires *before* the
   handler (`lan_stub` is `NoReturn`), exiting with "run `grid mode cloud`". The classification and
...
2. **One handler, `cli/cloud_grid.py:cmd_cloud_members`, branching on `args.subcommand`.** It mirrors
   `_add_engine_setup`'s nested-subparser idiom (`dest="subcommand"`, `required=True`, each
   subparser `set_defaults(handler=cmd_cloud_members)`). The parser points directly at the real cloud
   handler — the `login`/`logout` pattern for CLOUD_ONLY commands (there is no LAN handler to reroute
...
3. **Account-level auth = the session token; no status/relay call.** Membership is an account-level
   ...
   and makes **no** `get_managed_network_status` / relay call — the leanest mirror is `cmd_cloud_down`.
...
4. **Endpoint repointed to `/v1/grid/managed-networks/{id}/members`** (D11/D13), not the port
   ...
   `cloud/control_plane.py` and reuse `_client`/`_send`/`_raise`, so a transport error or any `≥400`
...
- The members wire contract joins the lifecycle clients in `cloud/control_plane.py` (mocked in tests
