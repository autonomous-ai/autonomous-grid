# ADR 0004 — Unified `grid join`: remote provider serve (single engine)
...
In local, `grid join` writes an engine record and spawns a detached `__engine` child that push-registers
the in-memory grid proxy and heartbeats it. Remote is the same verb with a different loop: bring the
...
Hard invariant: local `grid join` behaviour and the whole existing suite stay green; no off-local calls
leak into `shared/`/`local/`. One engine per `grid join` here — multi-engine routing (D9) is a later slice.

## Decisions

1. **New internal `__remote-engine <grid_id> <engine_id>` seam.** Dispatched in `cli/_main.py:_maybe_internal`
   into a new `remote/serve.py`; local's `__engine` → `cli/provider._run_engine` is untouched. The mode
...
2. **The engine record + `grid leave` are shared.** Record I/O and the SIGTERM→SIGKILL teardown moved
   ...
   no `local` import); `cli/provider.py` keeps its `_*` names as thin wrappers, so local behaviour and the
   existing test surface are byte-identical. Remote (`remote/serve.py`, `cli/remote_provider.py`) reuses the
   same module — no `remote → cli` back-dependency.
...
5. **Full live capability probe.** `remote/probe.py` (ported from the reference `engine/probe.py` +
...
6. **Relay contract = the reference paths**, base = the grid's `signaling_url`, `Authorization: Bearer
   ...
   `remote/relay.py` uses a **status-aware** policy (204/401/404 are outcomes, not fatal) — distinct from
   `control_plane._send`, which raises on every `≥400` — so the long-running loop refreshes / re-registers
   / backs off instead of dying. Heartbeat cadence is a fixed **30s** (the local `--heartbeat-interval`
   governs local only). The relay's poll response is **untrusted input**: the supplied `endpoint_path` is
...
7. **One engine, gated flags.** Remote `--all` and multi-engine routing (US 27) are rejected with
   guidance; auto-detect finding >1 engine errors. `--media` serving in remote mode is rejected for now (a later
   slice). The unified parser is one union; each handler validates per mode using the stamped `args.mode`
   seam: local `cmd_join` rejects the remote-only flags, remote `cmd_remote_join` rejects `--advertise-host` /
   `--media`. Remote-only flags default `None` so a wrong-mode use is detectable. **`--node-name` is
...
- local is untouched: the extraction is a pure refactor, the remote-only flags are rejected in local, and no
  new code runs for local users.
...
- The detached seam is the single plug-in point a future multi-engine / remote-media slice extends; the
