...
def act_gate():
    ...
    d.place("r1", "read-only A", "work", row=0, stage=2, note="full session, gated")
    d.place("r2", "read-only B", "work", row=1, stage=2)
    d.place("r3", "read-only C", "work", row=2, stage=2)
    d.place("sel", "select", "decide", row=1, stage=3)
    d.place("actor", "actor", "work", row=3, stage=3, note="the only one that may act")
    d.place("act", "act", "terminal", row=3, stage=4, note="one idempotent mutation, round_id-keyed")
    d.edge("job", "dot")
...
def lifecycle():
    ...
    d.place("job", "job", "terminal", row=1, stage=0)
    d.place("seat", "seat", "work", row=1, stage=1, note="resident session")
    d.place("warm", "warm", "work", row=0, stage=2, note="the layer's cache")
    d.place("hand", "handoff", "work", row=1, stage=2, note="same harness only")
    d.place("kill", "kill", "work", row=2, stage=2, note="frees the seat")
    d.place("snap", "snapshot", "work", row=1, stage=3, note="round_id-frozen, off-box")
...
def lanes():
    ...
    d.place("l1", "Codex exec", "work", row=0, stage=2, note="bounded coding")
    d.place("l2", "Claude Code", "work", row=1, stage=2, note="open work")
    d.place("l3", "Hermes ACP", "work", row=2, stage=2, note="verifiable")
    d.place("l4", "OpenClaw fan", "work", row=3, stage=2, note="N copies")
...
def seat_exec():
    ...
    d.place("bg", "background", "work", row=2, stage=1, note="shadow · warm · probe")
    d.place("pre", "preempt", "decide", row=2, stage=2, note="evict to snapshot, fsync first")
    d.edge("job", "live")
    d.edge("live", "ans")
    d.edge("bg", "pre")
...
def admission():
    ...
    d.place("job", "job", "terminal", row=1, stage=0)
    d.place("inc", "resident", "work", row=0, stage=1)
    d.place("can", "new harness", "work", row=2, stage=1, note="read-only shells only")
    d.place("cmp", "score", "decide", row=1, stage=2, note="vs the ground-truth authority")
...
def verifier():
    ...
    d.place("check", "check", "decide", row=1, stage=2, note="test · schema — a fact")
    d.place("cons", "consensus", "work", row=3, stage=2, note="two tails agree")
    d.place("ans", "answer", "terminal", row=1, stage=3)
...
def ledger():
    ...
    d.place("e1", "act", "work", row=0, stage=0)
    d.place("e2", "graduation", "work", row=1, stage=0)
    d.place("e3", "denial", "work", row=2, stage=0)
    d.place("app", "append", "decide", row=1, stage=1, note="round_id-stamped events")
    d.place("wal", "WAL", "deck", row=1, stage=2, sub="one box", note="append-only · no replication")
