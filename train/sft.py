...
def mlx_lora_config(cfg: TrainRunConfig, dest: Path) -> Path:
    """Write the one thing mlx-lm will not take on the command line: the LoRA size.

    `mlx_lm.lora` has no `--lora-rank` flag — rank and scale come from its own CONFIG_DEFAULTS
    (rank 8) or from a YAML passed with `--config`. So a workspace configured `lora_rank = 16`
    trained rank 8 on the Mac and rank 16 on torch, produced adapters of different sizes from the
    same config, and said nothing about it.

    `scale` is mlx-lm's multiplier where peft has alpha; peft's effective scaling is alpha / rank,
    so that is the conversion, and it keeps the two backends equivalent rather than merely
    configured alike. Written by hand — a three-line mapping is not worth a YAML dependency.
    """
    scale = round(cfg.trainer.lora_alpha / max(cfg.trainer.lora_rank, 1), 4)
    path = Path(dest) / "mlx-lora.yaml"
    path.write_text(
        "# generated by grid train sft — mlx-lm takes the LoRA size only through a config file\n"
        "lora_parameters:\n"
        f"  rank: {int(cfg.trainer.lora_rank)}\n"
        f"  scale: {scale}\n"
        "  dropout: 0.0\n",
        encoding="utf-8",
    )
...
def mlx_command(cfg: TrainRunConfig, data_dir: Path, adapter_dir: Path, *,
                iters: int, layers: int, config_file: Path | None = None) -> list[str]:
...
def run_sft_mlx(cfg: TrainRunConfig, examples: list[dict], run_dir: Path, *,
    ...
    command = mlx_command(
        ...
        config_file=mlx_lora_config(cfg, run_dir),
    )
