"""Cloud-mode `grid chat` / `image` / `edit` / `video`: requests through the active grid's relay.
...
local grid proxy, and accepts the cloud-only `--target-provider` / `--allow-self-provider` routing
...
Import rule mirrors `cli/cloud_grid.py`: `cloud.*` and the cloud-specific `cli` siblings are imported
...
def _resolve(args: argparse.Namespace) -> tuple[str, str, str]:
    """``(relay base, access token, grid label)`` for the active cloud grid, or a clean ``SystemExit``.

    Gates in order — signed in → a grid resolves → it has an access token → it is up — mirroring the
    guard in ``cli/cloud_provider.cmd_cloud_join``. The relay address comes from live status (creator)
    or the login bundle (member), via ``cloud_grid.resolve_relay_base``.
    """
    ...
    base, _status = cloud_grid.resolve_relay_base(session, rec, network_id, label)
...
def cmd_cloud_chat(args: argparse.Namespace) -> int:
    from cloud import relay
...
def cmd_cloud_image(args: argparse.Namespace) -> int:
    return _post_media(
        args,
...
def cmd_cloud_edit(args: argparse.Namespace) -> int:
...
def cmd_cloud_video(args: argparse.Namespace) -> int:
    return _post_media(
        args,
        "media/video/i2v",
        {
