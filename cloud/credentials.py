"""Cloud-mode credential store: the signed-in session + per-grid tokens.
...
def add_network(record: dict[str, Any]) -> None:
    """Register a cloud grid in the local store (e.g. one just created by ``grid up``).
...
def update_network_tokens(
    ...
    Used by the cloud serve loop after a relay 401 → token refresh. Immutable update: rebuilds the
...
def require_session() -> str:
    ...
    The auth gate every cloud command that needs identity calls before doing work.
    """
