"""Thin HTTP client for autonomous's hosted control plane (cloud mode).

Ported and trimmed from ``grid-src/grid_cli/control_plane.py``: the device-code sign-in
surface (start/poll), the post-login token fetch, and the cloud-grid lifecycle
...
API. Cloud mode is *allowed* to reach the internet — that is the feature — but nothing here
is reached in LAN mode (dispatch gates the cloud commands to cloud).
"""
...
def refresh_network_token(
    *, network_id: str, refresh_token: str, api_url: str | None = None
    ...
    """Exchange a per-grid refresh token for a fresh access token (relay auth, cloud serve loop).
...
def add_member(
    ...
    """Add (or update) a member of a cloud grid with the given role(s). Account-level — the session
...
def remove_member(
    ...
    """Remove a member from a cloud grid. ``email`` is percent-encoded into a single path segment so a
    stray ``/`` (or other path char) cannot re-target the request — the boundary ``network_id``'s
    regex guards in ``cli/cloud_grid.py``. A successful DELETE may answer ``204 No Content``."""
    with _client(api_url, session_token) as client:
        return _json_or_empty(_send(
            client, "DELETE",
...
def list_members(
    ...
    """The members of a cloud grid. Unwrap defensively: accept both the ``{"members": [...]}``
